...
/**
 * Java implementation for the wbladdsense API action.
 *
 * @author Elhuyar Foundation
 * 
 */
public class WblAddSenseToLexemeElhuyar {

	static final Logger logger = LoggerFactory
			.getLogger(WblAddSenseToLexemeElhuyar.class);
	...
	public WblAddSenseToLexemeElhuyar(ApiConnection connection, String siteIri) {
	...
	/**
	 * Executes the API action "wblAddSense" for the given parameters. Created
	 * or modified lexemes are returned as a result. In particular, this is
	 * relevant to find out about the id assigned to a newly created sense.
	 * <p>
	 *
	 * <p>
	 * See the <a href=
	 * "https://www.wikidata.org/w/api.php?action=help&modules=wbladdsense"
	...
	public void wblAddSense(String lexemeId, String data, boolean bot, long baserevid) throws IOException, MediaWikiApiErrorException {
		...
		JsonNode response = performAPIAction("wbladdsense", lexemeId, parameters, baserevid, bot); 
		
		//TODO Elhuyar Foundation delete this
		if(response.has("sense")){
			JsonNode sense=response.path("sense");
			if(sense.has("id")){
				//System.out.println("The sense for the lexeme has been successfully created. Sense id:"+sense.path("id"));
			}
		}else{
			System.out.println("The sense for the lexeme has not been successfully created.");
		}
		
		//TODO Elhuyar Foundation parse error
		//return getSenseDocumentFromResponse(response);
	...
	protected SenseDocument getSenseDocumentFromResponse(JsonNode root)throws IOException {
		...
		if (root.has("sense")) {
			return parseJsonResponse(root.path("sense"));
		} 
		else {
			throw new JsonMappingException("No sense document found in API response.");
		}
	...
	/**
	 * Parse a JSON response to extract an sense document.
	 * <p>
	...
	private SenseDocument parseJsonResponse(JsonNode entityNode) throws IOException {
		return mapper.readerFor(SenseDocumentImpl.class)
	 ...
	 * {@link WblAddSenseToLexemeElhuyar#editTimeWindow} time points when an edit was
